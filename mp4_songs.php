<?php
	...
	if(isset($_POST['upload'])){
		...
		if(isset($_FILES['video_file'])){
			...
			move_uploaded_file($file_tmp,"songs/mp4_songs/".$file_name);
		 }
		...
		$sql="CALL uploadsongs('$singer_id','$song_name','$song_format','$singer_name','$song_image','$video_file')";

		// $sql = "INSERT INTO upload_albums(`singer_id`,`song_name`,`song_format`,`singer_name`,`song_image`,`video_file`) VALUES('$singer_id','$song_name','$song_format','$singer_name','$song_image','$video_file')";
		$result = mysqli_query($conn,$sql);
		if($result){
	...
	while($rows=mysqli_fetch_array($result)){
		...
		if(isset($_POST[$song_id])){
			...
			$sql = "SELECT * FROM upload_mp4 WHERE song_id = '$song_id'  ";
			$results = mysqli_query($conn,$sql);
			...
			$sql = "INSERT INTO favorite_songs(`cat_id`,`song_id`,`user_id`,`song_name`,`singer_name`,`song_image`,`video_file`) VALUES('4','$song_id','$user_id','$song_name','$singer_name','$song_image','$video_file') ";
			$results = mysqli_query($conn,$sql);
	...
	<header>
      ...
      aria-haspopup="true" aria-expanded="false">Track</button>
								<div class="dropdown-menu dropdown-primary">
									...
									<a class="dropdown-item" href="mp4_songs.php"><b>Uploaded Songs</b></a>
                                    <a class="dropdown-item" href="mp4_songs.php"><b>MP4 Songs</b></a>
								</div>	
						</div>
						<li class="nav-item">
...
<form method="post" action="mp4_songs.php" enctype="multipart/form-data">
	<div class="form-row">
		<div class="col">
			<div class="form-group btn btn-primary">
				<input type="file" class="form-control-file" id="exampleInputFile" aria-describedby="fileHelp" name="video_file" required>
			</div>
		</div>		
		...
		<?php
			...
			if(mysqli_num_rows($result)>0){
				while($row = mysqli_fetch_array($result)){
					...
					echo "
							<div class='col-md-3'>
								<form method='post' action='mp4_songs.php'>
									<div class='card-deck'>
										<div class='card-img'>
											<img src='songs/mp4_songs/img/$song_image' style='width:350px;height:250px;' alt=''>
										</div>
										<div class='card-desc'>
											<h3>$song_name | $singer_name</h3>
											<video class='embed-responsive-item' controls='' preload='none'> <source src='songs/mp4_songs/$video_file' type='video/mp3'></video><br>
				...
				<div class="col-lg-6 contact-bottom d-flex flex-column contact-right-w3ls">
					...
					<div class="fv3-contact">
						<div class="row">
							...
							<div class="col-10">
								<h6>email</h6>
								<p>
									<a href="mailto:example@email.com" class="text-dark">vigneshvickky58@gmail.com</a>
								</p>
							</div>
						</div>
					</div>
					...
					<div class="fv3-contact">
						<div class="row">
							...
							<div class="col-10">
								<h6>address</h6>
								<p>DSI Labz | CHENNAI TAMILNADU</p>
							</div>
						</div>
					</div>
				</div>
